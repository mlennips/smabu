...
{
    internal class QuestReportFactory(IAggregateRepository repository, ReportsConfig config) : IReportFactory
    {
        public async Task<IReport> CreateInvoiceReportAsync(IEntityId id)
        {
            var invoice = await repository.GetByAsync((InvoiceId)id);
            var customer = await repository.GetByAsync(invoice.CustomerId);
        ...
        {
            var offer = await repository.GetByAsync((OfferId)id);
            var customer = await repository.GetByAsync(offer.CustomerId);
