...
namespace LIT.Smabu.UseCases.Invoices
{
    public static class ListInvoices
    {
        ...
        public class ListInvoicesHandler(IAggregateCache cache) : IQueryHandler<ListInvoicesQuery, ListInvoicesDTO[]>
        {
            public async Task<Result<ListInvoicesDTO[]>> Handle(ListInvoicesQuery request, CancellationToken cancellationToken)
            {
                IReadOnlyList<Invoice> invoices = request.CustomerId != null
                    ? await cache.ApplySpecificationTask(new InvoicesByCustomerIdSpec(request.CustomerId))
                    : await cache.GetAllAsync<Invoice>();

                IReadOnlyList<Payment> payments = await cache.ApplySpecificationTask(new PaymentsWithInvoiceIdSpec([.. invoices.Select(x => x.Id)]));
