...
namespace LIT.Smabu.UseCases.Offers
{
    public static class CreateOffer
    {
        ...
        public class CreateOfferHandler(IUnitOfWork uow, BusinessNumberService businessNumberService) : ICommandHandler<CreateOfferCommand, OfferId>
        {
            public async Task<Result<OfferId>> Handle(CreateOfferCommand request, CancellationToken cancellationToken)
            {
                ...
                await uow.Repository.CreateAsync(offer);
                return offer.Id;
            }
        }
    }
