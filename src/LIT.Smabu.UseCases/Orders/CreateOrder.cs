...
namespace LIT.Smabu.UseCases.Orders
{
    public static class CreateOrder
    {
        ...
        public class CreateOrderHandler(IUnitOfWork uow, BusinessNumberService businessNumberService) : ICommandHandler<CreateOrderCommand, OrderId>
        {
            public async Task<Result<OrderId>> Handle(CreateOrderCommand request, CancellationToken cancellationToken)
            {
                ...
                await uow.Repository.CreateAsync(order);
                return order.Id;
            }
        }
    }
