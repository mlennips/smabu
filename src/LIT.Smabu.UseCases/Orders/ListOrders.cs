...
namespace LIT.Smabu.UseCases.Orders
{
    public static class ListOrders
    {
        ...
        public class ListOrdersHandler(IAggregateCache cache) : IQueryHandler<ListOrdersQuery, OrderDTO[]>
        {
            public async Task<Result<OrderDTO[]>> Handle(ListOrdersQuery request, CancellationToken cancellationToken)
            {
                IReadOnlyList<Order> orders = await cache.GetAllAsync<Order>();
                Customer[] customers = await cache.GetByAsync(orders.Select(x => x.CustomerId));
                ...
                List<Offer> offers = offerIds.Any()
                    ? [.. (await cache.GetByAsync(offerIds))]
                    : [];

                List<Invoice> invoices = invoiceIds.Any()
                    ? [.. (await cache.GetByAsync(invoiceIds))]
                    : [];
