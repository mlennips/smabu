...
namespace LIT.Smabu.UseCases.Payments
{
    public static class DeletePayment
    {
        ...
        public class DeletePaymentHandler(IUnitOfWork uow, FinancialRelationsService financialRelationsService) : ICommandHandler<DeletePaymentCommand>
        {
            public async Task<Result> Handle(DeletePaymentCommand request, CancellationToken cancellationToken)
            {
                ...
                {
                    await uow.Repository.DeleteAsync(payment);
                }
