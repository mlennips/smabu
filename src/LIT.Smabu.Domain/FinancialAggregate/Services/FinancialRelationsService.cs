...
{
    public class FinancialRelationsService(IAggregateCache cache)
    {
        ...
        {
            AnnualFinancialStatement[] detectedFinancialStatements = await cache.ApplySpecificationTask(new DectecFinancialStatementByFiscalYearSpec(fiscalYear));
        ...
        {
            AnnualFinancialStatement[] detectedFinancialStatements = await cache.ApplySpecificationTask(new DectecFinancialStatementByPaymentIdSpec(paymentId));
            AnnualFinancialStatement? detectedFinancialStatement = detectedFinancialStatements.SingleOrDefault();
